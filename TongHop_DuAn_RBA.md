...
### 6.1. Bảng metrics đầy đủ (tập Test 75.000 dòng — toàn bộ ~500.000 dòng, chạy trên notebook (Colab))

| Chỉ số | Giá trị | Làm tròn | Ý nghĩa |
|---|---|---|---|
| **AUROC** | 0.8767 | **0.8767** | Khả năng phân biệt tổng quát 2 lớp (1.0 = hoàn hảo) — khá tốt |
| **AUPRC** | 0.2163 | **0.2163** | Đáng tin hơn AUROC khi dữ liệu mất cân bằng (~3% dương) |
| **Accuracy** | 0.7953 | **0.7953** | Tỉ lệ dự đoán đúng — dễ gây hiểu nhầm với dữ liệu lệch lớp |
| **Precision** | 0.1144 | **0.1144** | Trong các cảnh báo tấn công, ~11% là đúng thật |
| **Recall** | 0.8432 | **0.8432** | Trong các tấn công thật, bắt được ~84% |
| **F1** | 0.2014 | **0.2014** | Điều hòa giữa Precision và Recall |
...
**Số liệu bổ sung (chỉ có trong text `.docx`, KHÔNG có trong `metrics.json`)**: nếu chuyển sang **ngưỡng tối ưu F1 ≈ 0.856** thì Precision tăng lên **28.9%**, F1 = **0.322**, đổi lại Recall giảm còn **36.4%**.

### 6.2. Diễn giải ý nghĩa trong bối cảnh RBA

- **Recall cao (84.3%) + Precision thấp (11.4%)** là một sự đánh đổi *có chủ đích và hợp lý* cho bài toán bảo mật:
...
- **AUPRC (0.216) quan trọng hơn Accuracy/AUROC**: với dữ liệu lệch ~3% dương, một mô hình đoán "tất cả âm" đã đạt Accuracy ~97%, nên Accuracy 0.80 không phản ánh chất lượng thực. AUROC 0.877 trông đẹp nhưng cũng bị "thổi phồng" bởi lớp âm áp đảo. AUPRC 0.216 (so với baseline ngẫu nhiên ~0.03) mới cho thấy mô hình thực sự học được tín hiệu hữu ích.
...
- **Đã phát hiện & sửa 2 lỗi thực**:
  1. Hàm `low_med_high()` từng tính lại `xmin/xmax` từ batch truyền vào `transform()` thay vì dùng giá trị toàn cục lúc `fit()`. Với batch lớn ảnh hưởng không đáng kể, nhưng khi inference 1–2 dòng đơn lẻ thì `xmin=xmax=` giá trị mẫu → méo mức thuộc Low/High (bị đẩy về 1.0 giả tạo). **Đã sửa**: `fit()` lưu `self.minmax_` toàn cục của train; `transform()`/`_fuzzify()` dùng cố định; lưu `minmax_` vào pipeline để inference khôi phục đúng. Kiểm chứng inference: kịch bản "rủi ro cao" → **76.9%** (dự đoán 1), "rủi ro thấp" → **26.3%** (dự đoán 0).
  2. Rò rỉ thống kê toàn cục: trong `engineer_features()`, **median RTT** và **`value_counts` cho country/asn rarity** từng được tính trên **toàn bộ df** (vì `engineer_features` chạy *trước* `prepare_splits`), tức bao gồm cả val/test. **Đã sửa**: thêm `fit_global_stats()`/`apply_global_stats()` trong `features.py`, fit chỉ trên train, lưu `global_stats` vào pipeline, áp lại trong `inference.predict_risk`.
  - Đã huấn luyện lại sau cả 2 lần sửa (kết quả mục 6 là số liệu mới nhất). Metrics gần như không đổi so với trước khi sửa — đúng như dự đoán vì cả 2 dạng leakage đều nhẹ ở quy mô batch lớn (350.000 dòng train), nhưng pipeline giờ đúng nguyên tắc và đáng tin cậy hơn khi suy luận trên từng mẫu đơn lẻ.

### 8.2. Điểm yếu / rủi ro còn lại

- **Precision thấp (11.4%) ở ngưỡng 0.5**: nhiều báo động giả; chưa tối ưu ngưỡng ở khâu quyết định cuối.
- **Ngưỡng quyết định cứng 0.5**: dù đã xử lý mất cân bằng ở loss, mã không hạ/tối ưu ngưỡng theo F1 hay theo chi phí thực tế khi sinh `metrics.json` (ngưỡng F1 tối ưu 0.856 chỉ được nêu trong báo cáo, không áp dụng trong code đánh giá).
