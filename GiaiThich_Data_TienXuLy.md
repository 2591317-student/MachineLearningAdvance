...
>
> Mọi con số trong file đều lấy trực tiếp từ mã nguồn (`features.py`, `dataset_prep.py`,
> `fuzzy_mamdani.py`, `config.py`) và từ pipeline đã fit thật
> (`outputs/preprocessing_pipeline.pkl`), nên bạn có thể tin và dùng khi bị hỏi.
>
...
Năm bước, mỗi bước một nhiệm vụ rõ ràng:

| Bước | Làm gì | Vì sao cần |
|---|---|---|
| 1 | Đọc CSV, sắp xếp theo user + thời gian | Để biết "lần đăng nhập trước" của mỗi người là dòng nào |
| 2 | Sinh 19 đặc trưng số + 1 cột phân loại | Dữ liệu thô (tên quốc gia, chuỗi thời gian…) máy không học trực tiếp được |
| 3 | Chia Train/Val/Test = 70/15/15 | Cần một phần dữ liệu "chưa từng thấy" để đánh giá trung thực |
| 4 | Fit các bộ tiền xử lý **chỉ trên train** | Chống rò rỉ dữ liệu (mục 6) |
| 5 | Ghép thành vector 39 chiều | Đầu vào chuẩn cho MLP |
...
- → Phải dùng **AUPRC, Recall, Precision** (các chỉ số nhạy với lớp thiểu số).
- → Lúc huấn luyện phải **bù lệch lớp** bằng `pos_weight` trong hàm mất mát.
...
Ba việc:
1. Đọc file CSV.
2. Đổi cột thời gian từ *chuỗi văn bản* sang *kiểu ngày–giờ*, để có thể **trừ hai thời
   điểm cho nhau** (tính "cách lần trước bao lâu").
3. **Sắp xếp theo `User ID`, rồi theo thời gian.**

### Vì sao bước sắp xếp lại quan trọng đến vậy?

Vì ở bước sau, ta sẽ dùng `shift(1)` — nghĩa là "lấy giá trị của **dòng ngay phía trên**"
— để biết lần đăng nhập trước của user. Nếu dữ liệu không được sắp đúng thứ tự, "dòng phía
trên" sẽ **không phải** là lần đăng nhập trước đó, và **toàn bộ** đặc trưng lịch sử sai.
...
| Đặc trưng | Cách tính |
|---|---|
| `rtt_missing` | 1 nếu cột RTT bị thiếu (NaN) |
| `rtt_filled` | Giá trị RTT, chỗ thiếu điền bằng **trung vị của tập train** |
...
> ⚠️ Chú ý: `rtt_filled` **không** được tính ở bước này, mà để sang Bước 4 — vì nó cần
> "trung vị của tập train", tức là một con số **học từ dữ liệu**. Trung vị thật đã fit
> được: **535,0 ms**.
...
> Hai đặc trưng này cũng để sang Bước 4, vì "tần suất" là thống kê học từ dữ liệu.
...
Đây là chỗ nên hiểu rõ, vì hai loại này chống bằng hai cách khác nhau.
...
*Đã sửa:* tách thành hai hàm rõ ràng — `fit_global_stats(df_train)` chỉ **nhận tập train**,
và `apply_global_stats(df, stats)` để **áp** thống kê đó cho bất kỳ tập nào.
...
*Đã sửa:* lưu min/max **cố định** từ lúc `fit()` trên train (`self.minmax_`), và dùng lại
y hệt cho mọi lần suy luận sau.
...
Học **ngưỡng phân vị 20/50/80** và **min/max** cho 5 biến liên tục. Giá trị thật:
...
Đầu ra: **15 mức độ thuộc** (5 biến × 3 mức Low/Med/High) + **1 điểm rủi ro centroid**
= **16 đặc trưng mờ**. (Chi tiết 8 luật và cách ra điểm centroid: xem
...
Con số **39** này chính là `input_dim` của tầng đầu tiên trong MLP — khớp đúng với output
`Input dimension: 39` khi chạy notebook.

Toàn bộ bộ tiền xử lý (scaler, one-hot encoder, thống kê toàn cục, ngưỡng Mamdani, danh
sách cột) được **lưu lại thành một file pipeline** (`preprocessing_pipeline.pkl`). Lúc suy
luận, ta nạp lại đúng file này để biến đổi dữ liệu mới **y hệt** như lúc huấn luyện — nếu
không, mô hình sẽ nhận đầu vào có ý nghĩa khác hẳn và dự đoán sai.

---

## 8. Ví dụ xuyên suốt — một user với 3 lượt đăng nhập

Giả sử user **U** có 3 lượt (đã sắp xếp theo thời gian):
...
**Sau nhóm lịch sử (chỉ dùng quá khứ của U):**
...
**Sau khi áp thống kê toàn cục** (train: `freq(NO)=0,821`, `freq(US)=0,037`, `rtt_median=535`):
...
**Đưa 5 biến liên tục của L3 qua hệ mờ Mamdani** → 15 mức độ thuộc + 1 điểm rủi ro.
Với `country_rarity = 0,963` (dùng ngưỡng thật ở mục 6.2) → `high = 0,955` — tức hệ mờ
xác nhận "quốc gia này rất hiếm".
...
(0,963 so với 0,179). Đây đúng là kiểu lượt đăng nhập mà hệ thống RBA
cần chấm điểm rủi ro cao — và các đặc trưng ta thiết kế đã **nắm bắt được** đầy đủ tín hiệu đó.
...
Hệ quả cho hàm thuộc của biến này (đã kiểm chứng bằng code):
...
suy biến này một cách an toàn (có nhánh riêng `if a == b == c` trong hàm `triangular`), nên
không gây lỗi — nhưng đây là **một hạn chế trung thực** đáng nêu: mức phân giải của biến
này bị giới hạn bởi đặc điểm phân bố quá tập trung của dữ liệu.
...
## 10. Tóm tắt 7 điều cần nhớ

1. **Sắp xếp theo user + thời gian** trước tiên — nếu không, `shift(1)` lấy sai dòng và mọi
   đặc trưng lịch sử đều sai.
2. **Đặc trưng lịch sử chỉ dùng quá khứ** (`shift`, `cumsum − hiện tại`) → chống rò rỉ
   theo thời gian.
3. **Chia stratified 70/15/15** → tỉ lệ 3% dương giữ nguyên ở cả 3 tập (3,0611/3,0613/3,0613%).
4. **Fit mọi bộ tiền xử lý CHỈ trên train** (trung vị RTT, độ hiếm, ngưỡng Mamdani, scaler,
   one-hot); val/test chỉ được `transform` → chống rò rỉ giữa các tập.
5. **Giá trị lạ được xử lý an toàn:** Country/ASN lạ → `rarity = 1` (cực hiếm, hợp lý về
   bảo mật); thiết bị lạ → one-hot toàn 0, không lỗi.
6. **min/max của Mamdani lấy cố định từ train** → suy luận 1 dòng đơn lẻ vẫn cho kết quả đúng.
7. **Đầu vào MLP = 39 chiều** = 19 số + 4 one-hot + 16 mờ.
...
- So sánh giá trị hiện tại với giá trị của **lần đăng nhập ngay trước đó của cùng user** (`shift(1)`).
- Khác → 1, giống → 0.
...
**Q12. Trong bài toán này có mấy loại rò rỉ? Chống bằng cách nào?**
...
**Q14. Nhóm từng phát hiện lỗi rò rỉ nào? Sửa ra sao?** *(rất nên chủ động nhắc)*
- **Lỗi 1:** trung vị RTT và tần suất Country/ASN ban đầu tính trên **toàn bộ** dữ liệu (gồm val/test). → Sửa: tách hàm `fit_global_stats(df_train)` chỉ nhận train, và `apply_global_stats()` để áp cho các tập khác.
- **Lỗi 2:** min/max của hệ mờ Mamdani bị tính lại theo **từng lô** truyền vào. Chạy theo lô lớn thì không thấy sai, nhưng suy luận **1 dòng đơn lẻ** thì min = max = chính giá trị đó → hàm thuộc méo hoàn toàn. → Sửa: lưu min/max cố định từ `fit()` trên train, dùng lại y hệt khi suy luận.
...
**Q19. Vì sao mọi thứ "học từ dữ liệu" chỉ được fit trên train?**
- Nếu fit trên cả val/test thì mô hình gián tiếp đã biết phân bố của tập dùng để đánh giá nó → điểm số bị thổi phồng, không phản ánh khả năng tổng quát hóa.
- Quy tắc: **train → `fit_transform`; val/test → chỉ `transform`**.
- Trong đồ án: 4 bộ đều fit trên train — thống kê toàn cục, hệ mờ Mamdani, OneHotEncoder, StandardScaler.
...
- Hướng cải tiến: để **ANFIS tự học** hàm thuộc và ngưỡng thay vì đặt cố định (đã nêu trong phần Hướng phát triển).
...
- Hệ quả: với biến đó, Low và Med **không phân biệt được** (cùng = 1 cho lưu lượng Na Uy), chỉ High là có phân hóa. Code có nhánh xử lý riêng (`if a == b == c`) nên **không lỗi**, nhưng độ phân giải của biến bị giới hạn.
- Cách trả lời tốt: thừa nhận, giải thích nguyên nhân, nêu hướng cải tiến (ANFIS tự học ngưỡng, hoặc tính phân vị trên tập giá trị khác nhau thay vì trên toàn bộ dòng).
...
- SMOTE / Focal Loss là hướng có thể thử thêm — đã nêu trong phần Hướng phát triển và có trong tài liệu tham khảo.
...
3. **Gặp câu không chắc 100%:** trả lời đúng phạm vi mình biết rồi nói thẳng giới hạn
   (như Q25, Q26) — tốt hơn nhiều so với cố nói cho có rồi sai.
4. **Khi viết tay:** ưu tiên gạch đầu dòng ngắn hơn đoạn văn dài — giám khảo chấm nhanh.
5. **Có số liệu thật thì nêu** (535 ms, 82,1% Na Uy, 39 chiều, 350.000/75.000/75.000) —
